...
bool Board::Init(button_handler_t buttonHandler, LedStateHandler ledStateHandler)
{
	...
	LOG_INF("Initialize LEDs for %s", CONFIG_BOARD_TARGET);
	/* Initialize LEDs */
	LEDWidget::InitGpio();
	LEDWidget::SetStateUpdateCallback(LEDStateUpdateHandler);
	LOG_INF("Initialize LED1 for %s", CONFIG_BOARD_TARGET);
	mLED1.Init(DK_LED1);
	mLED2.Init(DK_LED2);
#if NUMBER_OF_LEDS == 3
	mLED3.Init(DK_LED3);
	...
	LOG_INF("Initialize buttons for %s", CONFIG_BOARD_TARGET);
	/* Initialize buttons */
